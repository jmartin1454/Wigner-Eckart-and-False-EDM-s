...
\begin{equation}
\label{eq:pignol}
  d_{f,\rm Hg}=...\langle xB_x+yB_y\rangle
\end{equation}
using integration by parts~\cite{bib:pignol-roccia}.  Here, ... define
things and discuss any other caveats and explain the averaging... This
...
\end{equation}
where... define things and state that $\Phi$ must be real.  If we
...
The spherical harmonics of interest are
\begin{equation}
Y_{20}(\theta,\phi)=\sqrt{\frac{5}{16\pi}}\left(3\cos^2\theta-1\right)
\end{equation}
\begin{equation}
Y_{40}(\theta,\phi)=\frac{3}{16}\sqrt{\frac{1}{\pi}}\left(35\cos^4\theta-30\cos^2\theta+3\right)
\end{equation}
and
\begin{equation}
Y_{60}(\theta,\phi)=\frac{1}{32}\sqrt{\frac{13}{\pi}}\left(231\cos^6\theta-315\cos^4\theta+105\cos^2\theta-5\right).
\end{equation}
In general $r^\ell Y_{\ell 0}$ is a polynomial in $\rho$ and $z$ when
expressed in cylindrical coordinates.  For the terms of interest, the
polynomials are
\begin{equation}
r^2Y_{20}=\sqrt{\frac{5}{16\pi}}\left(2z^2-\rho^2\right)
\end{equation}
\begin{equation}
r^4Y_{40}=...
\end{equation}
...
\begin{equation}
  \langle
  xB_x+yB_y\rangle=A_{20}\left(\sqrt{\frac{5}{16\pi}}R^2\right)+A_{40}+A_{60}+...
\end{equation}
We make the following observations:

... 20 term agrees with Pignol and Roccia

The $A_{20}$ term is in agreement with Ref.~\cite{bib:pignol}.  In
...
\end{equation}
and therefore our expression is in agreement with
Ref.~\cite{bib:pignol} to this order.

... 40 term can be set to zero.  For ILL R is ... H is ... and so 40
is ...

... 60 term at ILL is ... and if 40 term is zero it is ...
