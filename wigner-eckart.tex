...
\address[lspc]{Universite' Grenoble Alpes, CNRS, Grenoble INP, LPSC-IN2P3, Grenoble, France}
...
\end{equation}
where the $g_{\ell0}$ are expressed in units of pT/cm$^{\ell-1}$.
... The $g_{40}$ terms was analyzed and measured by the PSI
collaboration~\cite{bib:komposch} and found to be in agreement with
expectation.
...
Another possibility was therefore to keep the height of the cell the
same, which would alleviate any high-voltage issues.  In this case the
cell radius would need to shrink to $R=12$~cm$/\sqrt{2}\approx
8.5$~cm.  The negative aspect here is that the volume of the
experiment would shrink by a factor of almost eight.  In general, the
statistical precision of neutron EDM experiments is driven by the
neutron density achievable in the cell, and this choice would mean
that the number of neutrons loaded into the cell would be reduced by
the same factor.
